...
# Lista das 15 Áreas da Vida
AREAS_VIDA = [
    "1. Relações familiares (não contando as relações com o cônjuge, companheiro(a) ou filhos",
    "2. Casamento / Companheiro(a) / Relacionamento Afetivo", "3. Ser pai ou mãe (ou exercer esse papel)",
    "4. Amizades / Relações Sociais", "5. Trabalho / Carreira / Atividade Laboral",
    "6. Finanças / Dinheiro / Independência Financeira", "7. Educação / Aprendizagem",
    "8. Lazer / Bem Estar / Hobbies", "9. Espiritualidade / Sentido da Vida / Religião",
    "10. Vida em Comunidade / Cidadania / Direitos e Deveres", "11. Autocuidado (descansar, dormir, exercício físico e alimentação)",
    "12. Gerenciamento do Tempo / Disciplina", "13. Amor e intimidade / Sexualidade",
    "14. Coragem / Habilidades para correr riscos", "15. Percepção Emocional / Flexibilidade Cognitiva",
    "16. Estética (arte, música, literatura e beleza)"
]

# Lista de ~50 Características de Valores
LISTA_VALORES = [
    ...
    "11. Conformidade – Respeitar e obedecer regras e obrigações", "12. Cooperação – Ser cooperativo(a) e colaborar com os outros",
    "13. Coragem – Ser corajoso(a) ou persistir diante do medo, ameaça ou dificuldade",
    "14. Criatividade – Ser criativo(a) ou inovador(a)", "15. Curiosidade – Ser curioso(a), de mente aberta e interessado(a); explorar e descobrir",
    "16. Incentivo – Encorajar e recompensar comportamentos que valorizo em mim ou nos outros",
    "17. Igualdade – Tratar os outros como iguais a mim", "18. Empolgação – Buscar, criar e participar de atividades empolgantes ou estimulantes",
    "19. Justiça – Ser justo(a) comigo ou com os outros", "20. Boa forma – Manter ou melhorar minha saúde física e mental",
    ...
    "23. Amizade – Ser amigável e agradável com os outros", "24. Perdão – Perdoar a mim mesmo(a) ou aos outros",
    ...
    "29. Humor – Ver e apreciar o lado divertido da vida", "30. Humildade – Ser humilde ou modesto(a); deixar que minhas conquistas falem por si",
    "31. Diligência – Ser trabalhador(a), dedicado(a) e esforçado(a)", "32. Independência – Ser autossuficiente e escolher meu próprio modo de fazer as coisas",
    "33. Intimidade – Abrir-me e compartilhar-me emocional ou fisicamente em relações próximas",
    "34. Justiça (legal/social) – Defender a justiça e a equidade", "35. Bondade – Ser gentil, compassivo(a) e cuidadoso(a) comigo ou com os outros",
    ...
    "38. Ordem – Ser organizado(a) e manter a ordem", "39. Abertura de mente – Considerar outros pontos de vista e avaliar evidências de forma justa",
    "40. Paciência – Esperar calmamente pelo que quero", "41. Persistência – Continuar apesar de problemas ou dificuldades",
    "42. Prazer – Criar e proporcionar prazer a mim ou a outros", "43. Poder – Influenciar fortemente ou liderar os outros",
    "44. Reciprocidade – Construir relações com equilíbrio justo de dar e receber",
    "45. Respeito – Tratar a mim ou aos outros com respeito e consideração", "46. Responsabilidade – Ser responsável e assumir minhas ações",
    "47. Romance – Ser romântico(a); expressar amor ou afeição intensa", "48. Segurança – Proteger e garantir a segurança minha ou de outros",
    ...
    "52. Autocontrole – Agir de acordo com meus próprios ideais", "53. Espiritualidade – Conectar-me com algo maior do que eu",
    "54. Habilidade – Praticar e melhorar continuamente minhas habilidades e aplicá-las plenamente",
    "55. Apoio – Ser prestativo(a), encorajador(a) e disponível para mim ou para os outros", "56. Confiança – Ser confiável, leal, sincero(a) e consistente"
]
...
# Inicializar controle de etapas do paciente
if "etapa_paciente" not in st.session_state:
...
if modo == "Área do Paciente":
    st.title("🧭 Exercício de Mapeamento e Bússola de Valores")
    
    paciente_id = st.text_input("Digite seu Nome Completo:").strip().lower()

    if paciente_id:
        # Trava de segurança se o paciente já respondeu
        if not df_dados.empty and "Paciente_ID" in df_dados.columns:
            registros = df_dados[df_dados["Paciente_ID"] == paciente_id]
            if not registros.empty and "Concluido" in registros["Status"].values:
                st.info("✅ **Exercício Concluído!** Suas respostas foram enviadas com segurança e serão analisadas em conjunto em nossas próximas sessões!")
                st.stop()

        # PASSO 1: AVALIAÇÃO DAS ÁREAS E CLASSIFICAÇÃO DOS VALORES
        if st.session_state["etapa_paciente"] == 1:
            st.markdown("""
            ### O que são Valores?
            Valores são como **bússolas internas**. Eles representam aquilo que é mais importante para você como ser humano, 
            indicando as direções em que deseja caminhar. Diferente de *metas* (que têm um fim), valores são um *modo de viver contínuo*.
            """)

            st.info("""
            📌 **Como funciona este exercício:**
            1. **Parte 1:** Serão listadas as áreas da vida valorizadas pela maioria das pessoas. Classifique cada área de acordo com a sua visão pessoal,
            atribuindo notas de 1 a 10 sob 6 perspectivas diferentes.
            2. **Parte 2:** Você classificará uma lista de valores em grau de importância e definirá quais são os seus 3 valores principais (suas bússolas inegociáveis) e 1 valor secundário.
            """)

            with st.form("form_valores_completo"):
                st.header("PARTE 1: Avaliação das Áreas da Vida")
                st.markdown("""
                ...
                * **Preocupação:** O quanto você está preocupado com a possibilidade de esta área não progredir como você deseja?
                ...
                st.divider()
                st.header("PARTE 2: Bússola de Valores (Valores Inegociáveis)")
                st.write("Decida quais valores são Muito Importantes, Importantes ou Não Importantes para você")
                ...
                avancar = st.form_submit_button("Avançar para Escolha das Bússolas Principais ➡️")
    
                if avancar:
                    # Guardar dados temporariamente na sessão
                    st.session_state["temp_respostas_areas"] = respostas_areas
                    st.session_state["temp_classificacao"] = classificacao_valores
                    st.session_state["etapa_paciente"] = 2
                    st.rerun()
...
# -------------------------------------------------------------------
    ...
    if senha == SENHA_TERAPEUTA:
        if df_dados.empty or "Paciente_ID" not in df_dados.columns:
            ...
            pacientes = df_dados["Paciente_ID"].unique().tolist()
            paciente_sel = st.selectbox("Selecione o Paciente para Análise:", pacientes)
            ...
            if paciente_sel:
                ...
                st.subheader("1. Mapeamento Multidimensional das 15 Áreas")
                ...
                st.divider()
                st.subheader("2. Bússola de Valores do Paciente")
                ...
                c1.metric("1º Valor Principal", top1)
                c2.metric("2º Valor Principal", top2)
                c3.metric("3º Valor Principal", top3)
                c4.metric("4º Valor Secundário", top4)

                st.markdown("### Outros Valores Destacados como 'Muito Importante':")
                muito_imp_list = d_pac["Valores_Muito_Importantes"].iloc[0]
                if pd.notna(muito_imp_list) and muito_imp_list:
                    ...
                    st.write("Nenhum outro valor marcado como 'Muito Importante'.")

                st.divider()
                if st.button("Liberar Novo Preenchimento para este Paciente"):
                    df_dados = df_dados[df_dados["Paciente_ID"] != paciente_sel]
                    conn.update(data=df_dados)
                    st.success("Paciente liberado para preencher novamente!")
                    st.rerun()
